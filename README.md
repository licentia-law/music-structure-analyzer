...
> **Forked from [ChordMiniApp](https://github.com/ptnghia-j/ChordMiniApp) (MIT).**
> Extends ChordMiniApp with: beat-slot Pattern Tables, Scale Degree analysis, .md/.png/.pdf exports, and simple shared-password auth.
...
| Python | 3.10.x | Use `uv python install 3.10` if not available |
| npm | bundled with Node | confirmed by `package-lock.json` |
| uv (optional) | any | faster Python install; `pip install uv` |

### 1. Clone & submodules

```bash
git clone https://github.com/licentia-law/music-structure-analyzer.git
cd music-structure-analyzer
...
# Note: spleeter 2.3.2 conflicts with httpx>=0.20. Install spleeter separately:
#   .venv\Scripts\pip install spleeter==2.3.2 --no-deps

.venv\Scripts\python app.py
```

### 5. SongFormer service (port 8080)
...
Open `http://localhost:3000`, enter a YouTube URL, and confirm the analysis runs end-to-end.

> **First run note:** ML model weights are downloaded automatically at first request.
> Beat-Transformer / Chord-CNN-LSTM / ChordMini: `python_backend/models/` (submodules, pulled at runtime).
> SongFormer: HuggingFace Hub (`ASLP-lab/SongFormer`), cached to `~/.cache/huggingface/`.
> Allow 5–10 min on first request for downloads.
...
|---------|---------|--------|
| [ChordMiniApp](https://github.com/ptnghia-j/ChordMiniApp) | MIT | Copyright (c) 2025 ChordMini Project |
| [SongFormer](https://github.com/ASLP-lab/SongFormer) | CC BY 4.0 | ASLP-lab / SJTU |
| [Beat-Transformer](https://github.com/ptnghia-j/beat-transformer-model) | MIT | ptnghia-j |
| [Chord-CNN-LSTM](https://github.com/ptnghia-j/chord-cnn-lstm-model) | MIT | Copyright (c) 2023 Music X Lab |
| BTC (ChordMini) | MIT | ptnghia-j |
| madmom | CC-BY-NC-SA 4.0 | CPJKU / Widmer — non-commercial use only |
| MuQ weights | CC-BY-NC 4.0 | Tencent AI Lab — non-commercial use only |
